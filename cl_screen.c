...
void SCR_DrawAutoIDStatus (autoid_player_t *autoid_p, int x, int y, float scale)
{
	...
	if (scr_autoid.integer >= 2)
	{
		...
		if (armor_color != NULL) {
			...
			Draw_AlphaFillRGB(x - bar_length * scale, y - AUTOID_ARMORBAR_OFFSET_Y * scale, bar_length * 2 * scale, 4 * scale, ALPHA_COLOR(scr_autoid_armorbar_green_armor.color, 50));
			Draw_AlphaFillRGB(x - bar_length * scale, y - AUTOID_ARMORBAR_OFFSET_Y * scale, armor_length * 2 * scale, 4 * scale, RGBAVECT_TO_COLOR(scr_autoid_armorbar_green_armor.color));
		}
	}
