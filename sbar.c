...
static void Sbar_DeathmatchOverlay(int start)
{
	...
	if (scr_scoreboard_drawfps) {
		x += 4 * FONT_WIDTH;
		Draw_SStringAligned(x, y - 8, "fps", scale, alpha, proportional, text_align_right, x + FONT_WIDTH * 3);
		x += 4 * FONT_WIDTH;
	}
	else {
		x += 3 * FONT_WIDTH;
	...
	for (i = 0; i < scoreboardlines && y <= SCOREBOARD_LASTROW; i++) {
		...
		if (scr_scoreboard_drawfps) {
			...
			if (s->last_fps > 0 && !s->spectator) {
				snprintf(myminutes, sizeof(myminutes), "%i", bound(0, s->last_fps, 999)); // limit to 3 symbols int
				if (s->last_fps < 70) {
...
static void Sbar_TeamOverlay(void)
{
	...
	x += 4 * FONT_WIDTH;
	x += FONT_WIDTH;
	Draw_SStringAligned(x, y, "total", 1, 1, proportional, text_align_center, x + FONT_WIDTH * 5);
	x += 5 * FONT_WIDTH;
	x += FONT_WIDTH;
