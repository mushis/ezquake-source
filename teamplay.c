...
char* TP_AlignMacroText(char* text, int fixed_width, int alignment)
{
	...
	case TP_MACRO_ALIGNMENT_RIGHT:
		strlcpy(output, va("%*s%s", spaces, "", content), MAX_MACRO_STRING);
		break;
	case TP_MACRO_ALIGNMENT_CENTERED:
		strlcpy(output, va("%*s%s%*s", spaces / 2, "", content, (spaces + 1) / 2, ""), MAX_MACRO_STRING);
		break;
	case TP_MACRO_ALIGNMENT_LEFT:
	default:
		strlcpy(output, va("%s%*s", content, spaces, ""), MAX_MACRO_STRING);
		break;
	}
...
{
	cvar_t* width_cvar = Cvar_Find(va("tp_length_%s", cvar_ext));
	...
	*alignment = TP_MACRO_ALIGNMENT_LEFT;
	...
	if (width_cvar) {
		*fixed_width = max(0, min(width_cvar->integer, 40));
		...
		alignment_cvar = Cvar_Find(va("tp_align_%s", cvar_ext));
...
{
	char* cvar_ext = va("%s%s%c", extended ? "ext_" : "", isupper(character) ? "caps_" : "", character);
