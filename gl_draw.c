...
static byte crosshairdata[NUMCROSSHAIRS][81] = {
    {
    ...
    {
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff,
	0xff, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
	...
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xfe, 0xfe, 0xfe, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xfe, 0xff, 0xfe, 0xff, 0xfe, 0xff, 0xff,
	0xff, 0xfe, 0xfe, 0xff, 0xfe, 0xff, 0xfe, 0xfe, 0xff,
	0xff, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xff,
	0xff, 0xfe, 0xff, 0xfe, 0xfe, 0xfe, 0xff, 0xfe, 0xff,
	0xff, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xfe, 0xfe, 0xfe, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff
    },
	...
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff,
	0xff, 0xfe, 0xff, 0xff, 0xfe, 0xff, 0xff, 0xfe, 0xff,
	0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xfe, 0xfe, 0xfe, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff
...
void Draw_InitCrosshairs(void)
{
	...
	for (i = 0; i < NUMCROSSHAIRS; i++)
	{
		...
		crosshairtextures[i] = GL_LoadTexture (str, 9, 9, crosshairdata[i], TEX_ALPHA, 1);
	}
...
void Draw_Crosshair (void)
{
		...
		(customcrosshair_loaded & CROSSHAIR_IMAGE))
	{
		...
		qbool use_screen_coords = (customcrosshair_loaded & CROSSHAIR_IMAGE);

		if (!gl_crosshairalpha.value) {
			return;
		}

		if (!CL_MultiviewGetCrosshairCoordinates(use_screen_coords, &x, &y, &half_size)) {
			return;
		}

		// user adjustment to center
		if (use_screen_coords) {
			// backwards compat: allow user to keep specifying in text coords
			if (vid.width) {
				x += (glwidth / vid.width) * cl_crossx.value;
			}
			if (vid.height) {
				y += (glheight / vid.height) * cl_crossy.value;
			}
		}
		else {
			x += cl_crossx.value;
		...
		if (customcrosshair_loaded & CROSSHAIR_IMAGE) {
			...
			ofs1 = crosshairpic.width * 0.5 * bound(0, crosshairsize.value, 20) - 1;
			ofs2 = crosshairpic.height * 0.5 * bound(0, crosshairsize.value, 20) + 1;
			sh = crosshairpic.sh;
			sl = crosshairpic.sl;
			th = crosshairpic.th;
			tl = crosshairpic.tl;
			...
			ofs1 = 4.5; // built-in crosshairs are 9x9 now
			ofs2 = 4.5;
			tl = sl = 0;
			sh = th = 1;
